/**
 * @license AngularJS v1.2.0-536e3c8
 ...
 */
var version = {
  full: '1.2.0-536e3c8',    // all of these placeholder strings will be replaced by grunt's
...
// functions such as the Function constructor, thw global Window or Document object.  In addition, many powerful
// functions for use by JavaScript code are published on scope that shouldn't be available from within an Angular
// expression.
//
...
// access to any member named "constructor" or to any member whose name begins or ends with an underscore.  The latter
// allows one to exclude the private / JavaScript only API available on the scope and controllers from the context of an
// Angular expression.
//
// For reflective calls (a[b]), we check that the value of the lookup is not the Function constructor, Window or DOM
// node while evaluating the expression, which is a stronger but more expensive test. Since reflective calls are
...
function ensureSafeMemberName(name, fullExpression, allowConstructor) {
  ...
  if (name === "constructor" && !allowConstructor) {
    ...
    throw $parseMinErr('isecprv',
        'Referencing private fields in Angular expressions is disallowed! Expression: {0}', fullExpression);
  }
  return name;
};
